...
128 BPM, eight 4/4 bars, D minor, 2.5–17.5 seconds in a 20-second film.
"""
...
BEAT = 60 / BPM
START = 2.5
CUTS = [3.4375, 6.25, 9.0625, 13.75, 17.5]
...
# Two repetitions of Dm9 / Bbmaj7 / Fadd9 / Csus2; a newly composed motif.
chords = [[50, 57, 60, 64], [46, 53, 57, 60], [48, 53, 57, 67], [48, 55, 62, 67]]
roots = [38, 34, 41, 36]
motifs = [[74, 69, 77, 76, 69, 72], [74, 77, 81, 77, 72, 69],
          [77, 72, 79, 81, 77, 72], [79, 74, 76, 72, 67, 69]]
positions = [0, .75, 1.5, 2.25, 2.75, 3.5]
for bar in range(8):
    start = START + bar * 4 * BEAT
    chord_index = bar % 4
    add(music, pad(chords[chord_index], 4 * BEAT + .18), start, .095)
    for beat in range(4):
        # Last half-beat opens into the closing image.
        add(mix, kick(), start + beat * BEAT, .72 if bar else .61)
        if beat in [1, 3]:
            add(mix, clap(), start + beat * BEAT, .18)
        add(mix, hat(True), start + (beat + .5) * BEAT, .067, .20)
    for eighth in range(8):
        add(mix, hat(), start + eighth * BEAT / 2, .033 * (1 if eighth % 2 else .65), -.25)
    if bar in [3, 7]:
        for off in [3.25, 3.5, 3.75]:
            add(mix, hat(), start + off * BEAT, .045, -.1)
    for off, length, octave in [(0, .65, 0), (.75, .8, 0), (1.75, .65, 12), (2.5, .6, 0), (3.25, .65, 0)]:
        add(music, bass(roots[chord_index] + octave, length * BEAT), start + off * BEAT, .21)
    for index, (off, midi) in enumerate(zip(positions, motifs[chord_index])):
        strength = .14 if index in [0, 2, 3] else .105
        if bar >= 4:
            strength *= 1.12
        sound = pluck(midi)
        moment = start + off * BEAT
        pan = [-.23, .18, -.08, .24, -.20, .1][index]
        add(music, sound, moment, strength, pan)
        add(music, sound, moment + BEAT * .75, strength * .28, -pan)
        add(music, sound, moment + BEAT * 1.5, strength * .11, pan)

# Musical bus ducking: smooth recovery after each kick keeps transients clear.
t = timeline(DURATION)
phase = np.mod(np.maximum(t - START, 0), BEAT)
duck = 1 - .52 * np.exp(-phase / .085)
duck[t < START] = 1
...
output = Path(__file__).resolve().parents[1] / 'artifacts' / 'macbook-original-score.wav'
